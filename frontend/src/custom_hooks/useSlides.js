...
const useSlides = (currentIndex, slides, pagination) => {
    ...
    const buttons = (
        <GroupButton group_class="slide-buttons">
            <CustomButton
                onClick={handleBackward}
                icon="fas fa-arrow-left"
                btn_class={`${activeIndex !== 0 ? "btn-icon-text btn-back" : "invisible"} `} />
            {pagination && <span className="slide-pagination text-muted">{activeIndex + 1}/{slides.length}</span>}
            <CustomButton
                onClick={handleForward}
                icon="fas fa-arrow-right"
                btn_class={`${activeIndex + 1 === slides.length ? "invisible" : "btn-icon-text btn-forward"} `}
            />
        </GroupButton>
    )
