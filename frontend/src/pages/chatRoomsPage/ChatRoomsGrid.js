...
const ChatRoomsGrid = ({ chats }) => {
    const { push } = useHistory()
    const { username } = useParams();
    const [open, setOpen] = useToggle(false);

    const handleClick = (chatId) => {
        push(`/messages/${chatId}/`)
    }
        ...
        <SectionWrapper>
            <BoxWrapper>
                <section className="chats__userList">
                    {chats.length > 0 ? chats.map(({ _id, member, lastMessage, updatedAt }, index) => {
                        // const { member, lastMessage, updatedAt } = chat;
                        return (
                            <div key={index} onClick={() => handleClick(_id)}
                                className="chatHistory__item">
                                <Image src={member.profileImage || profile_blank} className="chatHistory__img" />
                                <Container className="chatHistory__content" >
                                    <span className="username">{member.username} </span>
                                    <p className="chatHistory__text">{lastMessage ? lastMessage.slice(0, 25) + "..." : ""}</p>
